...
import { faCoins, faReceipt, faBox, faCubes } from '@fortawesome/free-solid-svg-icons';
...
export default function SellerSalesAnalysis() {
  const [stats, setStats] = useState(null);
  const [loading, setLoading] = useState(true);

  useEffect(() => {
    api.get('/analytics/seller-sales').then((res) => setStats(res.data)).finally(() => setLoading(false));
  }, []);

  if (loading) return <p className="text-gray-700 dark:text-gray-300">Loading...</p>;
  if (!stats) return <p className="text-gray-500 dark:text-gray-400">Failed to load sales analysis.</p>;

  const costByDayData = stats.costByDay.map((d) => ({
  ...
  return (
    <div className="space-y-6">
      <h2 className="text-xl font-bold text-gray-900 dark:text-gray-100">Sales Analysis</h2>

      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        <StatCard icon={faCoins} label="Total Cost (successful orders)" value={formatRs(stats.totalCost)} />
        <StatCard icon={faReceipt} label="Successful Orders" value={stats.orderCount} />
        <StatCard icon={faCubes} label="Units Sold" value={stats.unitsSold} />
        <StatCard icon={faBox} label="Active Products" value={stats.productCount} />
      </div>

      <ChartCard title="Cost — last 30 days" empty={costByDayData.length === 0}>
      ...
      <ChartCard title="Top Products by Units Sold" empty={topProductsData.length === 0}>
        <ResponsiveContainer width="100%" height={260}>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
        ...
        <div className="bg-white dark:bg-neutral-900 dark:border dark:border-neutral-800 rounded-xl shadow-sm dark:shadow-none p-4 overflow-x-auto">
          <h3 className="font-bold text-gray-900 dark:text-gray-100 mb-3">Top Products (Cost Basis)</h3>
